import { Button, Card } from 'react-bootstrap'
import { useMenuBasket } from '../context/MenuBasketContext'
import { formatCurrency} from '../utilities/formatCurrency'

type MenuItemProps = {
    id: number
    name: string
    price: number
    decription: string
    imagen: string
}

export function MenuItem ({id, name, price, decription, imagen}: MenuItemProps) {
    const { getItemQuantity, increaseBasketQuantity, decreaseBasketQuantity, removeFromBasket } = useMenuBasket()
    const quantity = getItemQuantity(id)
    return <Card className='h-100'>
                <Card.Img variant='top' src={imagen} height="200spx" style={{ objectFit: "cover"}} />
                <Card.Body className='d-flex flex-column'>
                    <Card.Title className='d-flex justify-content-between align-items-baseline mb-4' >
                        <span className='fs-2'>{name}</span>
                        <span className='ms-2 text-muted'>{formatCurrency(price)}</span>
                    </Card.Title>                    
                    <Card.Text>{decription}</Card.Text>                 
                    <div className='mt-auto'>
                        {quantity === 0 ? (<Button variant='warning' className='w-100' onClick={() => increaseBasketQuantity(id)}>+ Add To Basket</Button>
                        ) : <div className='d-flex align-items-center flex-column' style={{gap: ".5rem"}}>
                                <div className='d-flex align-items-center justify-content-center' style={{gap: ".5rem"}}>
                                <Button variant='warning' onClick={() => decreaseBasketQuantity(id)}>-</Button>
                                <div>
                                    <span className='fs-3'>{quantity}</span> in Basket
                                </div>
                                <Button variant='warning' onClick={() => increaseBasketQuantity(id)}>+</Button>
                                </div>
                                <Button variant='danger' size='sm' onClick={() => removeFromBasket(id)}>Remove</Button>                               
                            </div>}
                    </div>
                </Card.Body>
            </Card>
}